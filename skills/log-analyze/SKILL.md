...
如果 AI 第一次使用本 skill，或发现 `env-map.json` / `env-map.md` 不存在或内容不完整，必须：

1. 每次只问用户一个问题
2. 先用一句人话说明：这是在补“常用项目名 / 机器简称 / 日志目录”的私有信息，后面查日志时你才能自动定位机器
3. 若 `env-map.json` / `env-map.md` 还不存在，优先先执行 `agentsshcli env-map init --from-config`，自动读取现有 JumpServer 连接；若已存在，就直接基于现有 `env-map.json` 增量维护，不要重复 init
4. 把交互问题收敛成 4 组：JumpServer 与环境对应关系、日志路径模式、项目简称/别名、一组常用主机列表
5. 不要一上来要求用户提供完整 hostname；优先接受项目简称、机器简称、业务名、实例尾号或 IP
6. 首次连通验证成功后，必须把返回的真实 hostname 回显给用户，再决定是否写入默认 target / 机器简称映射
7. 询问日志路径时，不要只说“映射”；应直接问“这个项目日志一般在 /www/{project}/logs 还是 /data/{project}/logs，或者别的真实路径”
8. 把收集结果优先写入当前目录的 `env-map.json`，再自动渲染 `env-map.md`
9. 写入后立刻用 `agentsshcli jump-exec` 做最小验证
...
2. 各环境日志通常在哪个路径模式
3. 你平时最常查的项目名或简称有哪些
4. 你是否愿意直接给一组常用主机列表（hostname、机器简称或 IP 都可以）
5. 如果用户给了主机列表，按小并发轻量验证并写回结构化映射；失败项单独标记后再确认
6. 如果用户没有主机列表，再退回“先拿一个最常用的机器线索做验证”
7. 首次验证成功后，把返回的真实 hostname 展示给用户，并补齐默认 target / 机器简称 / 项目别名

## 私有映射文件

当前用户的真实环境信息应写入：

- [env-map.json](./env-map.json)
...
在做任何远端命令之前，先优先读 `env-map.json`，必要时用 `env-map.md` 做人工核对，完成：
